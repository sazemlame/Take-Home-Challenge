...
def find_car_with_licence_plate(lplate):
    return next((index for (index, d) in enumerate(plot) if d["licence_plate_no"] == lplate), None)+1      #Extracting the slot number for the car parked with the given licence plate number
...
def get_lic_no_same_age(a):                                                                                #Finding licence number of cars with same age
    #print("Entering function for licence nos")
    list_of_cars=[]
    for cars in plot:
        if cars==None:
            continue
...
with open(filename,'r') as f:
    ...
    if x.split()[0].lower()!="create_parking_lot":                        #Creating the database. If the first line is not initializing the database then a message prompt is sent
        sys.exit("Please create a database")
    ...
    for y in f:                                                      #Reading the other lines of the command file
        ...
        if re.match('^Slot_number',y.split()[0]):                       #Checking if the command requries to return slot numbers
            ...
            nslots=None                                                      
            for s in y.split():
                if(re.match('[A-Z]{1,2}-[0-9]{1,2}-[A-Z]{1,2}-[0-9]{1,4}',s)):             #Extracting number plate no to search for the corresponding slot
